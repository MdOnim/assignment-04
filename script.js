...
function calculateCount() {
  ...
  if (currentStatus === "all-filter-btn") {
    currentTabCount = totalJobs;
  } else if (currentStatus === "Interview-filter-btn") {
    currentTabCount = Interviewbox.length;
  } else if (currentStatus === "Rejected-filter-btn") {
    currentTabCount = Rejectedbox.length;
  } else {
    currentTabCount = totalJobs;
  }
  jobsCount.innerText = `${currentTabCount} of ${totalJobs}`;
}
...
mainContainer.addEventListener("click", function (event) {
  if (event.target.classList.contains("interview-btn")) {
    ...
    const notApplicate = parentNode.querySelector(
      ".Not-Applied-button",
    ).innerText;
    ...
    const mobileShopExist = Interviewbox.find(
      (item) => item.mobileShop == cardInfo.mobileShop,
    );
    if (!mobileShopExist) {
      Interviewbox.push(cardInfo);
    }

    Rejectedbox = Rejectedbox.filter(
      (item) => item.mobileShop != cardInfo.mobileShop,
    );
    calculateCount();
    ...
    const mobileShopExist = Rejectedbox.find(
      (item) => item.mobileShop == cardInfo.mobileShop,
    );
    if (!mobileShopExist) {
      Rejectedbox.push(cardInfo);
    }
    Interviewbox = Interviewbox.filter(
      (item) => item.mobileShop != cardInfo.mobileShop,
    );
...
function renderInterview() {
  ...
  for (let interview of Interviewbox) {
    ...
    div.className =
      " mb-5  job-card flex justify-between border border-gray-300 rounded-sm shadow-lg bg-[#FFFFFF] md:gap-9";
    div.innerHTML = `
        ...
        <div class="p-5">
                ...
                <div>
                <p class="delete-btn p-2 mt-4 mr-4 border border-slate-300 rounded-full 
                 hover:bg-red-500 duration-500 cursor-pointer hover:text-gray-50"><i class="fa-regular fa-trash-can"></i></p>
                 </div>
